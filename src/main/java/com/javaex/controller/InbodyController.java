...
public class InbodyController {
	...
	// 회원이 본인 인바디 페이지를 볼 때
    @GetMapping("")
    public String inbody(HttpSession session,
        ...
        return getInbodyPage(authUser, authUser.getUserId(), crtPage, model);
    }

    // 트레이너가 회원의 인바디 페이지를 볼 때
    @GetMapping("/member/{memberId}")
    public String inbodyByTrainer(@PathVariable("memberId") int memberId,
                                  @RequestParam(value = "crtPage", required = false, defaultValue = "1") int crtPage,
        ...
        // TODO: 트레이너가 해당 회원을 볼 권한이 있는지 확인하는 로직 추가 필요
        
        return getInbodyPage(authUser, memberId, crtPage, model);
    }

    
    // 인바디 페이지에 필요한 데이터를 준비하고 뷰를 반환하는 공통 메소드
    private String getInbodyPage(UserVO authUser, int targetUserId, int crtPage, Model model) {
